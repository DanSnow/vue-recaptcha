...
export const RecaptchaContextKey =
  "vue-recaptcha-context" as unknown as InjectionKey<RecaptchaContext>;
...
export function useAssertV2SiteKey(): string {
  const ctx = useRecaptchaContext();
  invariant(
    ctx.options.v2SiteKey,
    "Your config is not compatible with recaptcha v2, please provide v2SiteKey",
  );
...
export function useAssertV3SiteKey(): string {
  const ctx = useRecaptchaContext();
  invariant(
    ctx.options.v3SiteKey,
    "Your config is not compatible with recaptcha v3, please provide v3SiteKey",
  );
